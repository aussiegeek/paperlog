...
function adifRecord() {
  ...
  const output = `
  // generated by scripts/adif_tables.ts
  import { optional, Describe, type, array, string } from "superstruct";
  import {
    Boolean,
    CreditList,
    Date,
  ...
  switch (type) {
    case "Enumeration":
      switch (enumeration) {
        ...
        case "Primary_Administrative_Subdivision[MY_DXCC]":
        case "Primary_Administrative_Subdivision[DXCC]":
          return "optional(PrimaryAdministrativeSubdivisionEnum)";
        case "Secondary_Administrative_Subdivision[DXCC]":
        case "Secondary_Administrative_Subdivision[MY_DXCC]":
          return "optional(SecondaryAdministrativeSubdivisionEnum)";
        default:
          console.log(enumeration);
          return `optional(${pascalCase(enumeration)}Enum)`;
      }

    case "CreditList,AwardList":
      return "optional(CreditList)";
    // fallthrough to types defined in types.ts
    case "Boolean":
    case "Date":
    case "GridSquare":
...
function genericEnumerationExtract(
  ...
  export const ${pascalCase(enumName)}Enum = enums(${enumName});
