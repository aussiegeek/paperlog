...
const enumTables = [
  ...
  },
  {
    value: "Code",
    text: "District",
    file: "secondary_administrative_subdivision_alt",
  },
  ...
  { value: "Status", text: "Description", file: "eqsl_ag" },
  { value: "Abbreviation", text: "Meaning", file: "morse_key_type" },
  { value: "Status", text: "Description", file: "qso_download_status" },
];
...
async function parseBands() {
  ...
  await Promise.all(
    ...
    `;
      await writeTypes("bands.ts", output);
    }),
  );
}

async function adifRecord() {
  const result = parseFile<{
    "Field Name": string;
    "Data Type": string;
    Enumeration: string;
  ...
  await writeTypes("adifRecord.ts", output);
  ...
  await writeTypes("adifRecordArbs.ts", testArbsOutput);
}
...
function mapTSAdifType(
  ...
  switch (normalisedType) {
    ...
    case "EqslAgEnum":
    case "MorseKeyTypeEnum":
    case "QsoDownloadStatusEnum":
